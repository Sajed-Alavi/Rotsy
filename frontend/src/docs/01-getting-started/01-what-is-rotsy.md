...
Rotsy is a DevSecOps intelligence platform: a single console over **GitHub**, **GitLab**, **SonarQube**, and **Sonatype Nexus Repository Manager**, with static container-image vulnerability scanning and automatic code analysis as its centrepiece.

Each of those tools is good at one job and bad at showing you the picture across all three. *Which repository's coverage just dropped? Which commit introduced a new vulnerability? Which images have critical CVEs, and what is eating disk?* Rotsy sits in front of GitHub, GitLab, SonarQube, and Nexus, orchestrates them, and answers those from one place — **Code Quality** is a global section over every synced repository and branch, and a **Project** additionally ties a repository together with its Nexus artifacts, instead of leaving you to check every tool separately.

Rotsy does not replace any of them. GitHub and GitLab still host your code, SonarQube still performs the static analysis, Nexus still stores the artifacts. Rotsy owns the orchestration, the correlation between them, and the product experience — including **Smart Insights**, which interpret what changed between two analyses instead of just displaying raw numbers.

## What it adds

| Capability | What it means |
|---|---|
| Automatic analysis | Push to GitHub or GitLab → Rotsy clones, runs SonarQube, and reports results — no CI YAML, no manual scan |
...
Rotsy does not replace Nexus and does not proxy artifact traffic. Your clients still `docker pull` from Nexus directly. It does not replace SonarQube's analysis engine or GitHub/GitLab's hosting — it drives sonar-scanner and the GitHub/GitLab APIs on your behalf, and keeps its own database of results, metrics and insights.
...
Start with [Your first login](/docs/first-login), then [Connecting Nexus](/docs/connecting-nexus), [Connecting GitHub](/docs/connecting-github) or [Connecting GitLab](/docs/connecting-gitlab), and [Connecting SonarQube](/docs/connecting-sonarqube).

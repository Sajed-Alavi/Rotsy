...
`_build_repo_payload` (`backend/app/routers/repositories.py`) now intersects `body.extra` with the keys it has already set and raises `HTTPException(400)` naming the collisions, before the merge. `extra` still works for genuinely new Nexus fields; it can no longer overwrite a validated one.

Testing Result:
`backend/tests/test_repositories.py` — `name`/`online`/`storage`/`docker` overrides each rejected with 400, the concrete blob-store repointing exploit rejected, and a non-colliding `cleanup` key still merged.

...
async def handle_clone_and_analyze(job: Job, progress: ProgressCallback) -> dict:
    ...
    try:
        ...
        await events.publish(events.AnalysisCompleted(
            project_id=project_id, analysis_run_id=run.id, sonar_project_id=sonar_project.id,
            repo_name=repo_name, ref=ref, commit_sha=sha, quality_gate=gate_status,
            issues_count=issues_count, bugs=bugs, vulnerabilities=vulnerabilities,
            code_smells=code_smells, coverage=coverage,
        ))
        ...
        await events.publish(events.AnalysisFailed(
            project_id=project_id, repo_name=repo_name, ref=ref, error=str(exc),
        ))
        raise
    finally:

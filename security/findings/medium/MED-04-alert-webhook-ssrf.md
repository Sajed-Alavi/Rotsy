...
New shared guard `backend/app/core/outbound.py::validate_outbound_url`: requires http/https, resolves the host and checks **every** returned address against loopback/private/link-local/reserved/multicast/unspecified ranges (which covers `169.254.169.254`), and fails closed when the host will not resolve. `OUTBOUND_ALLOWED_HOSTS` is the explicit admin escape hatch for on-prem destinations, empty by default.

Applied at both ends: a `field_validator` on `AlertCreate.webhook_url` and `AlertUpdate.webhook_url` so bad destinations never persist, and again inside `notifier.send_webhook` at delivery time — rules stored before this guard existed have never been validated, and re-checking closes the DNS-rebinding window.

Testing Result:
`backend/tests/test_outbound.py` — non-http schemes, empty/hostless URLs, all blocked IP ranges, a public-looking hostname resolving to RFC1918, and unresolvable hosts all rejected; allow-listed hosts permitted, and the allow-list shown not to open other private addresses.
